...
struct Secrets: Decodable {
    let oneTrustAPIKey: String
    
    static func load() -> Secrets? {
        // If you're getting an error here, you don't have a `secrets.json`.  Tsk tsk.
        let url = Bundle.module.url(forResource: "secrets.json", withExtension: nil)
        guard let url else { XCTFail("Secrets.load: Could find secrets.json in the test bundle."); return nil }
        let data = try? Data(contentsOf: url)
        guard let data else { XCTFail("Secrets.load: Could load data from \(url)"); return nil }
        do {
            let secrets = try JSONDecoder().decode(Secrets.self, from: data)
            return secrets
        } catch {
            XCTFail("Secrets.load: Encountered error, \(error)")
        }
        return nil
    }
}
